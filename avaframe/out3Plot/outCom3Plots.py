...
def avalancheThalwegPlot(ax, fieldRaster, dem, avaProfileMass, splitPoint=None, avalancheDir=""):

    indStart = avaProfileMass["indStartMassAverage"]
    indEnd = avaProfileMass["indEndMassAverage"]
    rowsMin, rowsMax, colsMin, colsMax = pU.constrainPlotsToData(
        fieldRaster, 5, extentOption=True, constrainedData=False, buffer=""
    )
    ...
    ax = outCom1DFA.addDem2Plot(ax, dem, what="hillshade", extent=extent)
    # add path
    ax.plot(
